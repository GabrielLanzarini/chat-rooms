...
export default function ChatRoom() {
    ...
    const getMessages = async () => {
        const { data } = await axios.get(`http://${config.API_IP}:5000/messages/${roomId}`)
        setServerData(data.messages)
    }
    const postMessages = async () => await axios.post(`http://${config.API_IP}:5000/messages/${roomId}`, { username: username, message: inputValue })
    ...
    const handleArrow = () => {
        const container = document.querySelector("#scroll")
        if (container.scrollTop <= container.scrollHeight - container.clientHeight - 20) return setVisibleArrow(false)
    ...
    useEffect(() => {
        ...
        window.addEventListener("beforeunload", () => socket.emit("roomExit", roomId))
        return () => {
    ...
    return (
        <div className="flex items-center justify-center w-screen h-screen flex-col gap-3 bg-slate-950">
            ...
            <div className="w-[60%] flex items-center justify-center relative">
                ...
                >
                    <svg className={`w-7`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        <path fill="none" d="M0 0h24v24H0z"></path>
